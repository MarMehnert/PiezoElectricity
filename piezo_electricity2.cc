...
template <int dim>
void CoupledProblem <dim>::setup_qph()
{
	...
	//----------------------------------------
	{
		...
		//counter always jumps the number of n_q_points
		unsigned int history_index = 0;
		for (typename Triangulation<dim>::active_cell_iterator cell =
				triangulation.begin_active(); cell != triangulation.end();
				++cell)
		{
			//pointer to i-th position in the vector
			//later on loops will run over n_q_points starting
			//from the respective position for every element
			...
			triangulation.begin_active(); cell != triangulation.end(); ++cell)
	{
		...
		//initialize all quadrature points
	...
	for (; cell != endc; ++cell)
	{
		...
		for (unsigned int q_point = 0; q_point < n_q_points; ++q_point)
		{
			...
			if (cell->material_id()==1)
			{
				...
				C[1][2][1][2] = C[1][2][2][1] = C[2][1][1][2] = C[2][1][2][1] = (Material::Coefficients::material_1::mu);
				C[0][2][0][2] = C[0][2][2][0] = C[2][0][0][2] = C[2][0][2][0] = (Material::Coefficients::material_1::mu);
				C[0][1][0][1] = C[0][1][1][0] = C[1][0][0][1] = C[1][0][1][0] = (Material::Coefficients::material_1::mu);
				...
				C_kelvin [3][3] = C_kelvin [4][4] =C_kelvin [5][5] = (Material::Coefficients::material_1::mu);
			...
			else
			{
				...
				C[1][2][1][2] = C[1][2][2][1] = C[2][1][1][2] = C[2][1][2][1] = (Material::Coefficients::material_2::mu);
				C[0][2][0][2] = C[0][2][2][0] = C[2][0][0][2] = C[2][0][2][0] = (Material::Coefficients::material_2::mu);
				C[0][1][0][1] = C[0][1][1][0] = C[1][0][0][1] = C[1][0][1][0] = (Material::Coefficients::material_2::mu);
...
template <int dim>
void CoupledProblem<dim>::update_qph ()
{
	FEValues<dim> fe_values(fe_cell,
			qf_cell,
			update_values |
...
void CoupledProblem<dim>:: compute_average_stress_defo_gradient(Tensor<4,dim> &C, Tensor<2,dim> &average_defo_grad, Tensor<2,dim> &average_piola_kirchhoff_stress, SymmetricTensor<2,dim> &average_kirchhoff_stress)
{
	...
	const unsigned int n_q_points = qf_cell.size ();
	const unsigned int n_f_q_points = qf_face.size ();
	std::vector<Tensor<1,dim>> q_p_normal_vectors (n_f_q_points);
	std::vector<Tensor<1,dim>> q_p_normal_vectors2;
	std::vector<Tensor<1,dim>> f_q_p_displacement(n_f_q_points);
	std::vector<Point<dim>> q_points(n_q_points);
	std::vector<Point<dim>> f_q_points(n_f_q_points);
	//map vertex_index with computed cauchy-stress tensor
	std::map<unsigned int, SymmetricTensor<2,dim> > map_vertexindex_cauchy_stress;
	std::map<unsigned int, Tensor<2,dim> > map_vertexindex_piola_kirchhoff_stress;
	//-----------------------------------------------------------------
	//Loop over all quadrature points in order to compute the averaged
	...
	for (; cell != endc; ++cell)
	{
		...
		{
			//compiler directive -> treat user_pointer as if it was
	...
	for (unsigned int i=0; i<dim; ++i)
	{
		...
		{
			deallog << average_defo_grad[i][j] << "\t";
		}
		deallog << "\n";
	}

	deallog << "\nAverage Piola Kirchhoff Stress:\n";
	for (unsigned int i=0; i<dim; ++i)
	{
		for (unsigned int j=0; j<dim; ++j)
		{
			deallog << average_piola_kirchhoff_stress[i][j] << "\t";
		}
		deallog << "\n";
	}
