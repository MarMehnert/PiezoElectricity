...
struct Test
{
	...
	// Directly related to solution field
	const int F; // Deformation gradient
	const int E; // Electric field vector
...
template<int dim>
class CoupledMicroscopicProblem:public CoupledProblem<dim>
{
public:
	CoupledMicroscopicProblem ()
: CoupledProblem<dim> ()
  {}

	virtual ~CoupledMicroscopicProblem () {}
...
protected:
	...
	{
		GridIn<dim> grid_in;
		grid_in.attach_triangulation (this->CoupledProblem.triangulation);
		std::ifstream input_file("Cube_mm.inp");

		grid_in.read_abaqus (input_file);

		typename Triangulation<dim>::active_cell_iterator
		cell = this->CoupledProblem.triangulation.begin_active(),
		endc = this->CoupledProblem.triangulation.end();
		for (; cell != endc; ++cell)
		{
			for (unsigned int f = 0; f < GeometryInfo<dim>::faces_per_cell; ++f)
			{
	...
	make_constraints (const unsigned int newton_iteration, const unsigned int timestep)
	{
		...
		{
			this->CoupledProblem.dirichlet_constraints.clear();
			this->CoupledProblem.dirichlet_constraints.reinit (this->CoupledProblem.locally_relevant_dofs);
			int n_components=this->CoupledProblem.n_components;
			FESystem<dim> fe_cell=this->CoupledProblem.fe_cell;
			{
				...
				// Back face
				VectorTools::interpolate_boundary_values(this->CoupledProblem.dof_handler,
						Parameters::boundary_id_front,
						Functions::ZeroFunction<dim>(n_components),
						this->CoupledProblem.dirichlet_constraints,
						this->CoupledProblem.fe_cell.component_mask(this->CoupledProblem.y_displacement));
				...
				VectorTools::interpolate_boundary_values(this->CoupledProblem.dof_handler,
						...
						Functions::ZeroFunction<dim>(n_components),
						this->CoupledProblem.dirichlet_constraints,
						fe_cell.component_mask(this->CoupledProblem.x_displacement)|fe_cell.component_mask(this->CoupledProblem.y_displacement)|fe_cell.component_mask(this->CoupledProblem.z_displacement));
				...
				VectorTools::interpolate_boundary_values(this->CoupledProblem.dof_handler,
						...
						this->CoupledProblem.dirichlet_constraints,
						fe_cell.component_mask(this->CoupledProblem.voltage));
			...
			this->CoupledProblem.dirichlet_constraints.close();
		}
		else
		{
			// Remove inhomogenaities
			for (types::global_dof_index d=0; d<this->CoupledProblem.dof_handler.n_dofs(); ++d)
				if (this->CoupledProblem.dirichlet_constraints.can_store_line(d) == true)
					if (this->CoupledProblem.dirichlet_constraints.is_constrained(d) == true)
						if (this->CoupledProblem.dirichlet_constraints.is_inhomogeneously_constrained(d) == true)
							this->CoupledProblem.dirichlet_constraints.set_inhomogeneity(d,0.0);
		}

		// Combine constraint matrices
		this->CoupledProblem.all_constraints.clear();
		this->CoupledProblem.all_constraints.reinit (this->CoupledProblem.locally_relevant_dofs);
		this->CoupledProblem.all_constraints.merge(this->CoupledProblem.hanging_node_constraints);
		this->CoupledProblem.all_constraints.merge(this->CoupledProblem.dirichlet_constraints, ConstraintMatrix::left_object_wins);
		this->CoupledProblem.all_constraints.close();
	}

void run()
{
	make_grid();
	this->CoupledProblem.setup_system();
	this->CoupledProblem.solve_timestep(this->CoupledProblem.time, this->CoupledProblem.ts);
}

};

template<int dim>
class CoupledMacroscopicProblem:public CoupledProblem<dim>
{
public:
	CoupledMacroscopicProblem ()
: CoupledProblem<dim> ()
  {}

	virtual ~CoupledMacroscopicProblem () {}
...
protected:
	...
	make_grid ()
	{
		TimerOutput::Scope timer_scope (this->CoupledProblem.computing_timer, "Make grid");
		...
		this->CoupledProblem.triangulation.refine_global (Parameters::n_global_refinements);
